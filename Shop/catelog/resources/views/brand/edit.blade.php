    ...
    <div class="card col-md-12 p-2">
        <div class="card-body">
            <form action="{{ route('admin.brand.update', $brand->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row g-2">
                    <div class="col-md-4">
                        <x-admin::form.picture-upload title="Icon" preview="{{ asset('storage/' . $brand->icon) }}"
                            name="icon" />
                    </div>

                    <div class="col-md-4">
                        <x-admin::form.picture-upload title="Banner" preview="{{ asset('storage/' . $brand->banner) }}"
                            name="banner" />
                    </div>

                    <div class="col-md-4">
                        <x-admin::form.picture-upload title="Picture" preview="{{ asset('storage/' . $brand->picture) }}"
                            name="picture" />
                    </div>

                </div>

                <x-admin::form.input name='name' required='true' value="{{ $brand->name }}" title="Name" />

                <x-admin::form.textarea name='description' value="{{ $brand->description }}" title="Description" />

                <x-admin::form.input name='order_id' type="number" value="{{ $brand->order_id }}" title="Sort Order" />
                ...
                <div class="card p-2">
                    ...
                    <x-admin::form.input name='meta_title' value="{{ $brand->meta_title }}" title="Meta Title" />

                    <x-admin::form.textarea name='meta_description' value="{{ $brand->meta_description }}"
                        title="Meta Description" />

                    <x-admin::form.input name='meta_keywords' value="{{ $brand->meta_keywords }}" title="Meta Keywords" />
                </div>
