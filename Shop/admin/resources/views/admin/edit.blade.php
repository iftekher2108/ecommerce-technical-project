    ...
    <div class="card col-md-8 p-2">
        <div class="card-body">
            <form action="{{ route('admin.admin.update', $admin->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <x-admin::form.picture-upload title="Picture" preview="{{ asset('storage/' . $admin->picture) }}"
                    name="picture" />
                <div class="row g-2">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <x-admin::form.input name='username' value="{{ $admin->username }}" title="Username"
                            readonly='true' />
                    </div>
                </div>

                <x-admin::form.input type="email" name='email' value="{{ $admin->email }}" title="Email"
                    readonly='true' />
